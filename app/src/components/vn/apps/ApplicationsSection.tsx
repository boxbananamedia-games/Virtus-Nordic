...
/**
 * /applikationer — the concept showcase, on its own page.
 *
 * The page chrome is deliberately untouched Virtus Nordic: cream paper,
 * Cormorant headings, the teal eyebrow. The only place the visual language
 * changes is inside the device glass. That contrast IS the argument the page is
 * making.
 *
 * Controlled by the route so a hero device on the homepage can deep-link
 * straight into its concept (/applikationer?app=<id>).
 */
export function ApplicationsSection({
  ...
  return (
    <section
      id="applikationer"
      className="mx-auto max-w-6xl scroll-mt-24 px-5 pb-16 pt-36 md:px-8 md:pb-24 md:pt-44"
    >
      <Reveal>
        <span className="label-eyebrow">{t.apps.label}</span>
        <h1 className="mt-3 max-w-3xl font-display text-4xl font-semibold leading-tight text-navy md:text-5xl">
          {t.apps.headline}
        </h1>
      ...
      >
        ...
        <div key={`${app.id}-copy`} className="vn-app-copy">
          ...
          <p className="mt-4 max-w-xl leading-relaxed text-navy/80">{pick(app.valueLine, lang)}</p>

          <div className="mt-8">
            <span className="label-eyebrow">{t.apps.fields.problem}</span>
            <p className="mt-2 max-w-xl leading-relaxed text-navy/75">{pick(app.problem, lang)}</p>
          </div>
